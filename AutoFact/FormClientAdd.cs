...
namespace autofact
{
    internal class FormClientAdd : Form
    {
        // ?? Dependencies ???????????????????????????????????????????????????????
        private readonly ClientViewModel _vm;

        // ?? Palette (mirrors Form1) ????????????????????????????????????????????
        ...
        // ?? Result ????????????????????????????????????????????????????????????
        public bool ClientAdded { get; private set; }

        // ?? Constructor ???????????????????????????????????????????????????????
        public FormClientAdd(Bdd bdd)
        {
            _vm = new ClientViewModel(bdd);

            // Reflect ViewModel errors and busy state in the UI
        ...
        private void BuildCard()
        {
            ...
            const int errorH  = 34;
            const int btnH    = 44;
            const int addrH   = 80;   // taller for address textarea

            // Calculate total card height from content
            int bodyH = (labelH + 6 + inputH) * 3   // Nom, Email, Tel
                      + gap * 3
                      + labelH + 6 + addrH           // Adresse
                      + gap
                      + errorH + gap
                      + btnH + padV;

            int cardH  = 56 + padV + bodyH;          // 56 = header bar
            int margin = 48;
            ...
            y += labelH + 6;
            (var wNom, txtNom) = MakeInput("ex. Dupont Martin", false, cxW, xOff, y);
            ...
            y += labelH + 6;
            (var wEmail, txtEmail) = MakeInput("ex. contact@société.fr", false, cxW, xOff, y);
            ...
            y += labelH + 6;
            (var wTel, txtTel) = MakeInput("ex. 06 12 34 56 78", false, cxW, xOff, y);
            ...
            y += labelH + 6;
            (var wAddr, txtAdresse) = MakeInput("Rue, ville, code postal…", true, cxW, xOff, y, addrH);
            ...
            btnAdd    = MakeButton("Ajouter",  xOff + halfW + 12, y, halfW, btnH, filled: true);
        ...
        private void SetBusy(bool busy)
        {
            ...
            btnAdd.BackColor  = busy ? cBlueDeep : cBlue;
            btnAdd.Invalidate();
        }
        ...
        private async void BtnAdd_Click(object? sender, EventArgs e)
        {
            ...
            // If not ok, _vm.PropertyChanged already called ShowError via the handler above
        }
